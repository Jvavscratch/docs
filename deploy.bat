...
rem 检查dist目录是否存在 - 注意：构建输出目录为 docs 目录下的 'dist'
if not exist "dist" (
...
echo 文档构建成功！
echo 下一步可以使用GitHub Actions自动部署到GitHub Pages，或者手动部署dist目录的内容。
echo 使用方法：
echo 1. 自动部署：提交代码到main分支，GitHub Actions将自动部署
echo 2. 手动部署：将dist目录的内容推送到gh-pages分支
...
echo 5. 推送到GitHub：git push -f git@github.com:Jvavscratch/docs.git master:gh-pages
